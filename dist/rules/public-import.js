...
exports.rules = {
    ...
    create(context) {
        const { alias = "", otherPublicPatterns = [], otherPublicImport = "testing", layersPlusOne = [], otherIgnoreLayer, ...enclosures } = (context.options[0] || {});
        const isOther = otherPublicPatterns.some((pattern) => micromatch_1.default.isMatch(context.filename, pattern));
        return {
            ImportDeclaration(node) {
                ...
                if (isOther) {
                    const isIgnore = otherIgnoreLayer
                        ? !otherIgnoreLayer.some((layer) => importTo.includes(layer))
                        : true;
                    if (isIgnore &&
                        ...
                        context.report({
                            ...
                            fix: (fixer) => fixer.replaceTextRange(importNode.source.range, fixImport(importTo, alias, layersPlusOne, enclosures)),
                        });
                    }
                }
                else {
...
function isEnclosure(importLayer, importSlice, enclosures) {
    const { sharedEnclosure = [], featuresEnclosure = [], pageEnclosure = [], widgetEnclosure = [], } = enclosures;
...
//# sourceMappingURL=public-import.js.map